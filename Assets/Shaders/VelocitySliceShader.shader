Shader "Unlit/VelocitySliceShader"
{
    Properties
    {
    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            half4 frag(Varyings IN) : SV_Target
            {
                ...
                float3 velocityLat = SAMPLE_TEXTURE3D(_VolumeTex, sampler_VolumeTex, uvw).xyz;
                float speedPhys = length(velocityLat) * _VelocityLatToPhys;

                float denom = max(_SpeedMax - _SpeedMin, 1e-6);
                float ratio = (speedPhys - _SpeedMin) / denom;
